...
// ASMLocalRateLimiterInformer provides access to a shared informer and lister for
// ASMLocalRateLimiters.
type ASMLocalRateLimiterInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1.ASMLocalRateLimiterLister
}

type aSMLocalRateLimiterInformer struct {
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.IstioV1().ASMLocalRateLimiters(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.IstioV1().ASMLocalRateLimiters(namespace).Watch(context.TODO(), options)
			},
		},
		&alibabacloudservicemeshv1.ASMLocalRateLimiter{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *aSMLocalRateLimiterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&alibabacloudservicemeshv1.ASMLocalRateLimiter{}, f.defaultInformer)
}

func (f *aSMLocalRateLimiterInformer) Lister() v1.ASMLocalRateLimiterLister {
	return v1.NewASMLocalRateLimiterLister(f.Informer().GetIndexer())
}
