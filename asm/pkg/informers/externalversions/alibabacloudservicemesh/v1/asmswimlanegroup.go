...
// ASMSwimLaneGroupInformer provides access to a shared informer and lister for
// ASMSwimLaneGroups.
type ASMSwimLaneGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1.ASMSwimLaneGroupLister
}

type aSMSwimLaneGroupInformer struct {
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.IstioV1().ASMSwimLaneGroups().List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.IstioV1().ASMSwimLaneGroups().Watch(context.TODO(), options)
			},
		},
		&alibabacloudservicemeshv1.ASMSwimLaneGroup{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *aSMSwimLaneGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&alibabacloudservicemeshv1.ASMSwimLaneGroup{}, f.defaultInformer)
}

func (f *aSMSwimLaneGroupInformer) Lister() v1.ASMSwimLaneGroupLister {
	return v1.NewASMSwimLaneGroupLister(f.Informer().GetIndexer())
}
