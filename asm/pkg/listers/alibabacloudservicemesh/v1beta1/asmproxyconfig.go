...
type ASMProxyConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ASMProxyConfig, err error)
...
// List lists all ASMProxyConfigs in the indexer.
func (s *aSMProxyConfigLister) List(selector labels.Selector) (ret []*v1beta1.ASMProxyConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ASMProxyConfig))
	})
	return ret, err
}
...
type ASMProxyConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ASMProxyConfig, error)
...
// List lists all ASMProxyConfigs in the indexer for a given namespace.
func (s aSMProxyConfigNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ASMProxyConfig, err error) {
...
// Get retrieves the ASMProxyConfig from the indexer for a given namespace and name.
func (s aSMProxyConfigNamespaceLister) Get(name string) (*v1beta1.ASMProxyConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("asmproxyconfig"), name)
	}
	return obj.(*v1beta1.ASMProxyConfig), nil
}
