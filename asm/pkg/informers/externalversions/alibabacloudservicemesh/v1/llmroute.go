...
// LLMRouteInformer provides access to a shared informer and lister for
// LLMRoutes.
type LLMRouteInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1.LLMRouteLister
}

type lLMRouteInformer struct {
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.IstioV1().LLMRoutes(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.IstioV1().LLMRoutes(namespace).Watch(context.TODO(), options)
			},
		},
		&alibabacloudservicemeshv1.LLMRoute{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *lLMRouteInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&alibabacloudservicemeshv1.LLMRoute{}, f.defaultInformer)
}

func (f *lLMRouteInformer) Lister() v1.LLMRouteLister {
	return v1.NewLLMRouteLister(f.Informer().GetIndexer())
}
